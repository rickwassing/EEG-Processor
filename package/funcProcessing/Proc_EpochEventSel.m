function [EEG, Warnings] = Proc_EpochEventSel(EEG, EventSelSettings, RejSettings, TimeSelSettings, Warnings)
% -----
...
if EventSelSettings.UseEpochTime
    ...
    if EventSelSettings.SeparateFiles
        for i = 1:length(EventSelSettings.Labels)
            ...
            tmpeeg(i) = pop_epoch(EEG, EventSelSettings.Labels(i), EventSelSettings.Interval); %#ok<*AGROW>
        end
        ...
        EEG = pop_epoch(EEG, EventSelSettings.Labels, EventSelSettings.Interval);
    end
    % ---------------------------------------------------------
        ...
        for i = 1:length(EEG)
            if isfield(EEG(i).event, 'is_reject')
                ...
                if isempty(RejEpochs)
                    ...
                    Warnings = [Warnings; {sprintf('Did not remove any epochs, no rejected epochs found in dataset %i of %i\n', i, length(EEG))}];
                    Warnings = [Warnings; {'-----'}];
                ...
                Warnings = [Warnings; {sprintf('Did not remove any epochs, no rejected epochs identified in dataset %i of %i\n', i, length(EEG))}];
                Warnings = [Warnings; {'-----'}];
            end
        end
    end
    ...
    if EventSelSettings.SeparateFiles
        ...
        [PointOnset, PointDuration] = idx2bouts(SelIdx & TimeSelIdx & ~RejIdx);
        EEG = pop_select(EEG, 'point', [PointOnset, PointOnset+PointDuration]);
    end
    % Update the JSON Struct
    for i = 1:length(EEG)
