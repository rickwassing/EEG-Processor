function [ArgOut, Next, Warnings] = ProcSelectData(app, File, Settings) %#ok<INUSL>
% ---------------------------------------------------------
% Initialize
ArgOut = [];
Next = 'none';
...
Settings.EEG = pop_loadset('filepath', filepath, 'filename', [filename, extension]);
% ---------------------------------------------------------
...
if ~Settings.Split.Do && ~Settings.EventSel.Do && Settings.Reject.Epochs
...
for i = 1:length(Settings.EEG)
    ...
    Settings.EEG(i).filepath = ['./derivatives/EEG-select/sub-', KeysValues.sub, '/ses-', KeysValues.ses];
end
% ---------------------------------------------------------
