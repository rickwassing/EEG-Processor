...
for fname = {'var', 'corr', 'hurst', 'lfp', 'hfp'}
    if length(eegChans) < EEG.trials
        for chan = 1:size(stats.(fname{:}), 1)
            stats.(fname{:})(chan, isnan(stats.(fname{:})(chan, :))) = nanmean(stats.(fname{:})(chan, :));
        end
    else
        for ep = 1:size(stats.(fname{:}), 2)
            stats.(fname{:})(isnan(stats.(fname{:})(:, ep)), ep) = nanmean(stats.(fname{:})(:, ep));
        end
    end
end
% =========================================================
...
% =========================================================
    function [LFP, HFP] = GetLowHighFrequencyPower(EEG)
        ...
        LFP = squeeze(nanmean(powerSpectrum(:, LFrange, :), 2));
        HFP = squeeze(nanmean(powerSpectrum(:, HFrange, :), 2));
    end
% =========================================================
    function EEG = EpochEEG(EEG, eegChans, reject, epochlength)
        % ---------------------------------------------------------
        % Remove any rejected events
        if reject
