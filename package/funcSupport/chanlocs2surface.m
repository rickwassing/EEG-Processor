function chanlocs2surface(filepath, chanlocs, doPlot)
% --------------------------------------------------
% Code adapted from 
...
% SUBFUNCTIONS
% --------------------------------------------------
    function plotSurface(P, clocs, x, y)
        % --------------------------------------------------
        ...
        Ax.YColor = 'w';
        Ax.ZColor = 'w';
        h = triplot(P, '-k.', 'MarkerSize', 20, 'MarkerFaceColor', 'r', 'MarkerEdgeColor', 'r');
        % --------------------------------------------------
        ...
        for j = 1:length(h.XData)
            ...
            idx = find(x == h.XData(j) & y == h.YData(j));
            XData(j) = clocs(idx).Y;
            YData(j) = clocs(idx).X;
            ZData(j) = clocs(idx).Z;
        end
