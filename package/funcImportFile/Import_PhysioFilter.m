...
chanIdx = strcmpi({EEG.chanlocs.type}, 'EOG') | strcmpi({EEG.chanlocs.type}, 'VEOG') | strcmpi({EEG.chanlocs.type}, 'HEOG');
...
% If any channels have been found ...
if any(chanIdx)
    fprintf('>> BIDS: 10 to 100 Hz bandpass filter on %i rectified EMG channels\n', sum(chanIdx))
...
% If any channels have been found ...
if any(chanIdx)
    fprintf('>> BIDS: 0.03 to 100 Hz bandpass filter on %i nasal pressure channels\n', sum(chanIdx))
    % Extract the channels and filter the data
    tmppns = pop_firws(...
        pop_select(EEG, 'channel', find(chanIdx)), ...
        'fcutoff', [0.03, 100], ...
...
% If any channels have been found ...
if any(chanIdx)
    fprintf('>> BIDS: 10 to 100 Hz bandpass filter on %i snoring channels\n', sum(chanIdx))
    % Extract the channels and filter the data
    tmppns = pop_firws(...
        pop_select(EEG, 'channel', find(chanIdx)), ...
        'fcutoff', [10, 100], ...
