...
if Settings.DoSpectrogram
    try
        ...
        if length(IncludeChans) < 4
            disp('Did not calculate spectrogram, majority of expected channels were rejected')
            Warnings = [Warnings; {'Did not calculate spectrogram, majority of expected channels were rejected'}];
            Warnings = [Warnings; {'-----'}];
            return
        end
        % Try to find all the good 10-20 channels
        IncludeChanIdx = find(ismember({EEG.chanlocs.labels}, IncludeChans));
        % If not all are found, check if there are less then 17 channels and use them, otherwise trow a warning
        if length(IncludeChanIdx) ~= length(IncludeChans)
            if sum(strcmpi({EEG.chanlocs.type}, 'EEG')) <= 16
                IncludeChanIdx = find(strcmpi({EEG.chanlocs.type}, 'EEG'));
                if ~isempty(IncludeChanIdx)
                    disp('Did not calculate spectrogram, did not find any EEG channel')
                    Warnings = [Warnings; {'Did not calculate spectrogram, did not find any EEG channel'}];
                    Warnings = [Warnings; {'-----'}];
                    return
                end
            else
        ...
        if Settings.SpectrogramSettings.DoBaselineNorm
            ...
            EEG.specnormvals.mu = nanmean(EEG.specdata(:, :, BaselineIdx, :), 3);
